...
public sealed class GlyphAtlas
{
    ...
    private static readonly byte[] FontBitmap =
    [
    ...
    public byte[] CreateTextureData()
    {
        var data = new byte[Columns * Rows * GlyphWidth * GlyphHeight];
        var glyphCount = FontBitmap.Length / GlyphHeight;
        for (var glyphIndex = 0; glyphIndex < glyphCount; glyphIndex++)
        {
            ...
            {
                var destination = ((row * GlyphHeight + y) * Columns + column) * GlyphWidth;
                data[destination] = FontBitmap[(glyphIndex * GlyphHeight) + y];
            }
        }
    ...
    public bool IsPixelSet(char character, int x, int y)
    {
        ...
        return (FontBitmap[(GetGlyphIndex(character) * GlyphHeight) + y] & (1 << (7 - x))) != 0;
    }
}
