...
{
    // dsh-anchored-standard 的 eternal-minimal 三个 .mjs 已 vendor 到 Fixtures/(GitHub 在测试环境不可达,且 Linux 路径在 win32 不存在)。
    private static readonly string PluginRepo = Path.Combine(AppContext.BaseDirectory, "Fixtures", "eternal-minimal");

    [Fact]
    public async Task Compose_ActivatesPluginsAndGateway()
    {
        Assert.True(Directory.Exists(PluginRepo), $"vendored fixtures missing: {PluginRepo}");
        var dir = Path.Combine(Path.GetTempPath(), $"dsh-configboot-{Guid.NewGuid():N}");
        Directory.CreateDirectory(dir);
        HarnessApp? app = null;
        try
        {
            foreach (var file in new[] { "eternal-minimal.mjs", "instruction-hint.mjs", "compaction-epoch.mjs", "custom-bash.mjs" })
            ...
            // win32 上 bash 由 custom-bash 提供、pwsh 启用;其余平台 bash 来自内置 tool-bash、pwsh 行禁用。
            var expectedTools = OperatingSystem.IsWindows()
                ? new[] { "bash", "pwsh", "read", "write", "edit", "glob", "grep", "todo_write", "str_replace_editor" }
                : new[] { "bash", "read", "write", "edit", "glob", "grep", "todo_write", "str_replace_editor" };
            ...
            // 直接 await 其 fiber,而不是只信 Compose 扫描后的瞬时 State——并行负载下注册/激活之间存在观察窗口。
            var hintRuntime = Assert.Single(app.Ctx.Registry.Values().Where(runtime => runtime.Name == "instruction-hint"));
            foreach (var fiber in hintRuntime.Fibers)
                await fiber.Await();
            Assert.All(hintRuntime.Fibers, fiber => Assert.Equal(FiberState.Active, fiber.State));
            ...
            Assert.Equal(OperatingSystem.IsWindows()
                ? new[] { "bash", "pwsh", "str_replace_editor" }
                : new[] { "bash", "str_replace_editor" }, names);
