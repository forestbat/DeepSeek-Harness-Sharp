# AGENTS.md
1. 永远不要把“举例说明”当成全部情况，当用户举例时，一定要思考如何泛化，或者提问用户泛化的边界。
2. 不要新增只被调用一次的小 helper；只有当它能命名清楚概念、隔离复杂逻辑、复用已有边界或改善测试时才抽函数。
3. 作为对规则2的补充，如果同样的逻辑出现超过3次，就应考虑作为共享函数，并写入项目记忆以供继续复用（如果有项目记忆的话）。
4. 新模块应围绕职责和变化原因命名，避免按“工具集合”“杂项”“common”堆放无关能力。
5. 不要假设自己处在沙箱(sandbox)网络中，这会让你认为自己无法访问网络，干扰测试。
6. 代码分析、修改和重构应使用Rider自带的mcp工具代替read/find/grep等默认工具。
7. 建立sub agent进行任务时，**务必**撰写提示词，让sub agent也使用Rider mcp工具代替read/find/grep等默认工具。
8. 当需要验证某个函数/组件的行为、排查bug、或确认代码路径时，优先用Rider自带的xdebug_*工具，只有当MCP通讯受阻时，才应使用非调试器方式运行测试，以避免打断工作流。
9. 启动调试器/xdebug_*工具的资源占用并不高，因此除非用户明确要求，否则不得以**任何理由**逃避、跳过、结束调试，不得私自移除断点，一定要分析到断点的所有信息才能resume。
10. xdebug工具不能建立run configuration，因此如果要运行新的调试任务，需要使用`rider-run-config` skill创建run configuration。
11. 分析代码时，最好使用Rider analyze_calls和get_symbol_info工具而不是读大片codebase。
12. 编写代码之后需要运行rider_lint_files（或其同类的工具）进行代码检查，按照Rider的建议修正代码，Weak级别的警告不需要管，除非能够简化代码或提升性能。
13. 本仓库代码频繁改动，**注释可能与实际代码逻辑不一致**。判断函数行为时**一律以代码实现为准**，不得仅凭注释下结论；改动前必须读代码核实当前真实行为。
14. **测试代码不得复制/重写业务逻辑**。测试只负责组装输入（数据、文件路径、配置）和断言输出，**完全复用**业务逻辑，不重新实现业务逻辑，也不可自己绕过业务逻辑。
15. 只为可独立重用或复杂到需要隔离的逻辑编写单元测试。对于其余逻辑，对整个功能进行集成测试或组件测试可以提供更好的覆盖率，且维护成本更低。
16. 如果用户的话里存在多个问题，**一定要写 TODO 清单或并行开启sub agent**，以防上下文固定于一个特定的问题而丢弃其他问题。
17. 优化prompts应注意简短和通用性，不可将当前文档或问题限定的内容完全输入提示词。
18. 编写测试代码或者**任何可运行的文件**时，**绝对禁止**将文件输出到项目文件夹以外的路径。
19. 编写总结、计划、方案、报告、备忘时，**务必**使用中文文件名并输出到项目路径下/plans文件夹，禁止输出到/tmp路径。
20. 调试和分析问题时，应一次分析、修改、验证完毕，不要只分析然后说"仍需实施"，或者只改代码就报称修复。每次修复后必须用单元测试/集成测试/组件测试/断点调试验证实际效果，确认问题真正解决才能报完成。
21. 集成测试/组件测试必须全部通过才能运行主流程。
22. 禁止使用环境变量作为配置项。
23. 禁止修改代码格式（除非影响代码逻辑，例如不当的缩进导致编译错误或者if分支执行不完整），这会造成无谓的时间和token消耗；格式规约由dotnet format/csharpier钩子自动进行。
24. officecli是python-docx的超集，无论是检查还是处理docx文档信息，都应该使用officecli，禁止使用python-docx，也不许转成pdf或者用libreoffice解析，只是增加不必要实体。
25. 除非用户要求，否则禁止使用任何打分机制和置信度机制，因为没有理论指导如何消费这些分数。
26. 写提示词和注释的时候，一句话一行，例如：
    ```text
    你需要做如下工作，做完报告给我。
    ```
    禁止出现明明是一句话却要强行换行的情况：
    ...
    做完报告给我。
    ```
27. **确认废弃的代码一律删除**，不要想着"保留工作"或者"兜底"。死代码不只是垃圾，还会吸附后续修复接错线。重构时先查调用方，确认零调用即删；测试、参数、兼容层随机制一并删除。
28. 使用getter/setter操作属性时，应使用`field`语法，示例：
    ```csharp
    ...
    ```
29. 一个方法最好不要超过20条语句。
30. 用插值字符串代替字符串拼接（因为C#编译器对此有优化）。示例：
    ```csharp
    // GOOD
    ...
    ```
31. 对于小型、频繁传递的不可变数据，例如坐标、颜色或日期范围，使用struct/record struct代替class。
32. 若初始化逻辑平凡（或者说仅起到字段赋值作用而无其他逻辑），优先使用主构造函数（primary constructor）代替一般构造函数：
    ```csharp
    ...
    ```
33. 对于无法直接编辑的第三方库，可以考虑使用扩展块（C# 14引入）和更早之前的扩展方法：
    ```csharp
...
34. 在泛型类或方法中，不要与 object 类型进行来回转换，而应使用 where 约束或 as 运算符来指定泛型参数的确切特性。例如：
    ```csharp
    class SomeClass {}
...
35. LINQ表达式的返回结果应进行物化，例如：
    ...
    ```
36. 何时使用IEnumerable<T>/Span<T>（栈上）/Memory<T>（堆上）？
    ```
    需要处理数据吗？
...
37. 代码中的字面量（魔数）应以常量声明封装（若用于日志记录和追踪除外），例如：
    ```csharp
    ...
    ```
38. 避免使用嵌套的try catch块，这会减弱可读性。
39. 不要使用ref/out参数，这会降低可读性，返回复合对象、结构体或元组。
    例外情况：
    `bool success = int.TryParse(text, out int number); //使用了TryParse或者依赖库里本身就有ref/out参数，需要保证逻辑正确`
40. 若字符串含有大量转义字符，需要用原始字符串字面量代替：
    ```csharp
    string pattern = "^(https?:\\/\\/)(www\\.)?[a-zA-Z0-9]+\\.[a-z]+$"; //存在大量转义字符！需要放弃
    string pattern = """^(https?:\/\/)(www\.)?[a-zA-Z0-9]+\.[a-z]+$"""; //原始字符串字面量用"""开始和结束，转义字符减少很多
    ```
41. async/await用于I/O密集型任务，Task.Run()/Task.StartNew()则用于CPU（计算）密集型任务。
42. 建议直接await ValueTask或者ValueTask<T>，且只等待一次：
    ```csharp
    // OK / GOOD
    ...
    ```
43. 应尽量使用C#新语法或者更规范的语法，包括且不限以下的内容：
    ```csharp
...
44. C#支持解构元组，错误和正确示例如下：
    ...
    ```
45. 不要使用#region标记。
46. 写注释时禁止写入被否定的决策，除非它对应的代码曾在真实的运行中出现过bug。例如：
    ```
    // 注释：白米粥里需要加入大便……
    用户驳回：白米粥里为什么要加入大便？
    ...
    ```
47. 在写C#时，除非确实需要控制反转，否则不要总是试图用依赖注入去写逻辑。
48. 作为规则41的补充，涉及I/O的逻辑（例如Console.WriteLine/Console.Error.WriteLine，或者日志记录方法）应采取其异步重载（例如await Console.WriteLineAsync）。
49. 使用TUI/GUI进行测试时，禁止使用mock LLM和mock TUI，一律使用真实的LLM请求和项目下的TUI构建。
50. 如果接收到空的用户输入，不必管它，继续当前的工作。
